...
class Node:
    ...
    def set_plan_range_by_weeks_OLD(self, weeks_count: int, plan_year_st: int):
        """実週長に合わせて PSI バッファを初期化（子にも伝搬）"""
        self.plan_year_st = int(plan_year_st)
        # 参照用に plan_range は保持（≒表示や互換用途）が、ロジックは配列長で回す
        self.plan_range   = math.ceil(weeks_count / 53)
        ...
        for child in self.children:
            child.set_plan_range_by_weeks(weeks_count, plan_year_st)
    ...
    def set_plan_range_by_weeks_OLD(self, weeks_count: int, plan_year_st: int):
        """
        週数から直接 plan_range を逆算して PSI バッファを初期化
        """
        plan_range = (weeks_count + 52) // 53  # 切り上げ

        print(f"[DEBUG] {self.name} - set_plan_range_by_weeks: weeks_count={weeks_count}, plan_range={plan_range}")
...
# ****************************
    ...
    def copy_demand_to_supply_OLD(self): # in backward planning process
    ...
    def get_set_childrenP2S2psi_OLD(self, plan_range):
    ...
    def extract_CPU_OLD(self, csv_writer):
    ...
    def dump_PSI_psi4demand_OLD(self):
    ...
    def calcPS2I_decouple4supply_OLD(self):
