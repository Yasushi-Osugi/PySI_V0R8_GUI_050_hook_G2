...
def simulate(root, horizon_weeks, seed, overrides):
    # 1) ブリッジが run_core を持っていれば本体を呼ぶ
    try:
        import real_engine_bridge as bridge
        if hasattr(bridge, "run_core"):
            bridge.run_core(root, horizon_weeks, seed, overrides or {})
            return
    except Exception as e:
        print("[engine_api] real engine unavailable, fallback:", e)
