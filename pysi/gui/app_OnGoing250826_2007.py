...
# …が意外とデリケートなポイント
    ...
    def _clear_map_highlights(self):
        ...
        # ヒット点
        hit = getattr(self, "_map_hit_dot", None)
        if hit is not None:
            try: hit.remove()
            except Exception: pass
        ...
        # 注釈
        ann = getattr(self, "_map_annot", None)
        if ann is not None:
            try: ann.remove()
            except Exception: pass
    ...
    def _on_map_click(self, event):
        ...
        # “度”ベースのしきい値を現在の表示範囲から動的に決める
        if used_cartopy:
            import cartopy.crs as ccrs
            xmin, xmax, ymin, ymax = ax.get_extent(crs=ccrs.PlateCarree())
        else:
            xmin, xmax = ax.get_xlim(); ymin, ymax = ax.get_ylim()
        thr = max((xmax - xmin) * 0.01, (ymax - ymin) * 0.01)  # 画面の約1%

        # 最寄りノード探索（lon/lat 同士）
        hit, best = None, 1e9
        for name, (lon, lat) in pos.items():
            d = abs(lon - x) + abs(lat - y)  # マンハッタン距離で十分
            if d < best:
                hit, best = name, d
        ...
        # ---- ヒット点マーカー ----
        if used_cartopy:
            self._map_hit_dot = ax.plot(
                pos[hit][0], pos[hit][1], marker='o', ms=7,
                color='crimson', mec='white', mew=0.8,
        ...
        if used_cartopy:
            self._map_annot = ax.annotate(
                info, xy=pos[hit], xytext=(8, 10),
                textcoords="offset points",
                bbox=dict(boxstyle="round,pad=0.25", fc="w", ec="#888", alpha=0.9),
                fontsize=9, color="#222",
                transform=ax.projection, zorder=7
            )
        else:
            self._map_annot = ax.annotate(
                info, xy=pos[hit], xytext=(8, 10),
        ...
        # ---- ヒットノードに接続するエッジを太線で強調 ----
        lines = []
        def _plot_edge(u, v, color="#ff4444", lw=2.4, z=5):
        ...
        # 物理グラフがあればそこから両方向の接続を拾う
        import networkx as nx
        G = getattr(self, "G", None)
        if isinstance(G, (nx.Graph, nx.DiGraph)) and hit in G:
            if hasattr(G, "edges"):  # nx.DiGraph
                for u, v in G.out_edges(hit):
                    _plot_edge(u, v)
                for u, v in G.in_edges(hit):
                    _plot_edge(u, v)
            else:
                # 保険：無向グラフ
                for u, v in G.edges():
    ...
    def _on_map_scroll_OLD(self, event):
        """ホイールでズーム（カーソル位置を中心に）。Cartopy でも平面でも動く。"""
        ax = getattr(self, "_map_ax", None)
        ...
        used_cartopy = getattr(self, "_map_used_cartopy", False)
        if used_cartopy and getattr(self, "_map_data_crs", None):
            data_crs = self._map_data_crs
            # 現在の表示範囲（経緯度系で取得）
            ...
            ax.set_extent([cx - new_w/2, cx + new_w/2, cy - new_h/2, cy + new_h/2], crs=data_crs)
        else:
            # 通常 axes
            xmin, xmax = ax.get_xlim(); ymin, ymax = ax.get_ylim()
            cx, cy = event.xdata, event.ydata
    ...
    def _event_lonlat(self, event):
        """Matplotlibイベント座標を lon/lat(PlateCarree) に変換して返す。"""
        if event.xdata is None or event.ydata is None:
            return None
        ...
        # event は「軸の投影座標」なので → PlateCarree (lon/lat) に逆変換
