...
class PSIPlannerApp:
    ...
    # クラス PSIPlannerApp 内に追加
    def _ensure_psi_area(self, parent=None):
        if getattr(self, "scrollable_frame", None) and self.scrollable_frame.winfo_exists():
            return
        import tkinter as tk
        from tkinter import ttk
        parent = parent or getattr(self, "right_panel", None) or self.frame

        psi_frame = ttk.LabelFrame(parent, text="PSI Graph")
        psi_frame.pack(fill="both", expand=True, padx=4, pady=2)

        canvas = tk.Canvas(psi_frame, highlightthickness=0)
        vsb    = ttk.Scrollbar(psi_frame, orient="vertical", command=canvas.yview)
        ...
        self.psi_panel        = psi_frame
        self.canvas_psi       = canvas
    ...
    def on_product_changed(self, event=None):
        # 選択取得（values が空の場合の保険も）
        selected = None
        try:
            if hasattr(self, "cb_product") and self.cb_product["values"]:
                selected = self.cb_product.get()
        except Exception:
            pass

        if not selected and getattr(self, "product_name_list", None):
            selected = self.product_name_list[0]

        self.product_selected = selected
        print(f"[INFO] product_selected = {self.product_selected}")

        # 製品ルートの差し替え（あれば）
        if hasattr(self, "_apply_selected_product"):
            self._apply_selected_product(selected)

        # ビュー更新
        if getattr(self, "view_mode", "network") == "worldmap":
            self.show_world_map(selected)
        else:
            if hasattr(self, "view_nx_matlib4opt"):
                self.view_nx_matlib4opt()

        # PSI 更新
        if selected and hasattr(self, "show_psi_by_product"):
            self.show_psi_by_product("outbound", "demand", selected)
    ...
    def setup_ui(self):
        ...
        # --- View メニュー ---
        self.view_mode = "network"  # or "worldmap"
        view_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="View", menu=view_menu)

        view_menu.add_command(
            label="Network Graph",
            command=lambda: (setattr(self, "view_mode", "network"), self.show_network_graph())
        )
        view_menu.add_command(
            label="World Map (highlight product)",
            command=lambda: (setattr(self, "view_mode", "worldmap"),
                            self.show_world_map(self.product_selected))
        )

        # World Map フィット トグル（self 保持）
        self.world_map_fit_var = getattr(self, "world_map_fit_var", None) or tk.BooleanVar(value=True)
        self.world_map_fit = bool(self.world_map_fit_var.get())
        view_menu.add_checkbutton(
            label="World Map: Fit to data",
            variable=self.world_map_fit_var,
            onvalue=True, offvalue=False,
            command=self._toggle_worldmap_fit
        )
        ...
        # =====================================================================
        # 1) レイアウトを明示：左右パネルを最初に作る（←重要）
        # =====================================================================
        self.main_panel  = ttk.Frame(self.root)
        self.main_panel.pack(fill="both", expand=True)

        self.left_panel  = ttk.Frame(self.main_panel)    # ネットワーク／世界地図用
        self.right_panel = ttk.Frame(self.main_panel)    # 右カラム（上：パラメータ／下：PSI）
        ...
        # ---------- 右カラムは上下 2 分割（grid） ----------
        self.right_panel.grid_rowconfigure(0, weight=0)   # ← コントロール（固定高さにする）
        self.right_panel.grid_rowconfigure(1, weight=1)   # ← PSI（余り全部を取る）
        self.right_panel.grid_columnconfigure(0, weight=1)

        # === A) 右上：コントロールを「固定高さのスクロール領域」にする ===
        CTRL_H = 360  # ここは好みで調整（300〜420あたり）

        # Canvas（高さ固定）＋ Scrollbar
        self.ctrl_canvas = tk.Canvas(self.right_panel, highlightthickness=0, height=CTRL_H)
        self.ctrl_vsb    = ttk.Scrollbar(self.right_panel, orient="vertical", command=self.ctrl_canvas.yview)
        self.ctrl_canvas.configure(yscrollcommand=self.ctrl_vsb.set)

        # grid へ配置（row=0）
        self.ctrl_canvas.grid(row=0, column=0, sticky="ew", padx=0, pady=(0, 4))
        self.ctrl_vsb.grid(row=0, column=1, sticky="ns")

        # キャンバスの中に実体フレーム（ここにコントロール群を pack していく）
        ...
        # 互換のため、従来の self.frame は「右上の中身フレーム」を指す
        self.frame = self.right_ctrl

        # === B) 右下：PSI スクロール領域の入れ物 ===
        self.right_psi = ttk.Frame(self.right_panel)
        # columnspan=2 にして、Scrollbarぶんの列もまたいで広く使う
        self.right_psi.grid(row=1, column=0, columnspan=2, sticky="nsew", padx=0, pady=(0, 0))
        ...
        # =====================================================================
        # 右パネル：各種パラメータ UI（従来通り self.frame を親にしてOK）
        # =====================================================================
        ...
        self.Demand_Pl_button = ttk.Button(
            self.frame, text="Demand PlanMult", command=self.demand_planning4multi_product
        )
        self.Demand_Pl_button.pack(side=tk.TOP)

        self.Demand_Lv_button = ttk.Button(
            self.frame, text="Demand Leveling", command=self.demand_leveling4multi_prod
        )
        ...
        # =====================================================================
        # 5) 初回描画の順序を固定（ここに置くのがベスト）
        #    1) 左右の“器”を用意 → 2) on_product_changed()
        # =====================================================================

        # 左＝ネットワーク（または地図）の Axes/Canvas を用意
        self._ensure_network_axes(self.left_panel)

        # 右＝PSI スクロール領域を用意
        #self._ensure_psi_area(self.right_panel) # NG
        # PSI は right_psi の中に作る（親が right_panel だと grid/pack 混在になる）
        self._ensure_psi_area(self.right_psi)

        # plan_range などの既定値（安全網）
        ...
        # ★ 初回描画（set()/current() ではイベントが飛ばないため明示呼び出し）
        self.on_product_changed()

        # =====================================================================
        #right_panel の中身はすべて grid。
        #right_ctrl と right_psi の内部では pack を使う（親が別なので混在にはならない）。
        #self.frame を right_ctrl に向けることで、以降の pack() を安全に使えます。
        # =====================================================================
        # 旧レイアウト（plot_frame / network_frame / psi_frame）の生成は不要：
        # 二重化・衝突の原因になるので削除。
        # =====================================================================
...
# *************************
    ...
    def show_psi_OLD(self, bound, layer):
        ...
        # ルートの存在チェック（getで安全に）
        root_out = getattr(self, "root_node_outbound", None)
        root_in  = getattr(self, "root_node_inbound", None)
        if bound == "outbound" and root_out is None:
            print("[WARN] show_psi: root_node_outbound is None; abort.")
            return
        if bound == "inbound" and root_in is None:
            print("[WARN] show_psi: root_node_inbound is None; abort.")
            return
        ...
        if bound == "outbound":
            traverse_nodes(root_out)
        else:
            traverse_nodes(root_in)

        # データが空なら描画をスキップ（安全）
        if not psi_data:
            print("[INFO] show_psi: psi_data is empty; skip plotting.")
            return
...
# PSI graph 
# *************************
    def show_psi_by_product_OLD(self, bound, layer, product_name):
        ...
        for ax, (node_name, revenue, profit, profit_ratio, line_plot_data_2I, bar_plot_data_3P, bar_plot_data_0S) in zip(axs, psi_data):
            ...
            # 例）各ノード n のサブプロット ax を描いた直後
            # node_dict = nodes_allでnodeを取り出す
            # node = self.global_nodes[node_name]
            #@250821 ADD
            node = None
            if bound == "outbound":
                node = self.nodes_prod_outbound.get(node_name)
            elif bound == "inbound":
                node = self.nodes_prod_inbound.get(node_name)
            else:
                print("bound error")

            if node is not None:
                self.overlay_price_on_axes(ax, node, self.product_selected)
...
# …が意外とデリケートなポイント
    ...
    def _install_map_interactions(self):
        """地図イベントの重複登録を防ぎつつ、必要なコールバックを入れる。"""
        if not hasattr(self, "_map_cids"):
            self._map_cids = []
        ...
        if not hasattr(self, "canvas_network"):
            return

        cid1 = self.canvas_network.mpl_connect("button_press_event", self._on_map_click)
        cid2 = self.canvas_network.mpl_connect("scroll_event",        self._on_map_scroll)
        cid3 = self.canvas_network.mpl_connect("key_press_event",     self._on_map_key)
        self._map_cids.extend([cid1, cid2, cid3])

        # （任意）マウス中ボタンでパン、右ドラッグでパンにしたい場合は Toolbar を出す案もあります
        # from matplotlib.backends.backend_tkagg import NavigationToolbar2Tk
        # if not hasattr(self, "_map_toolbar"):
        #     self._map_toolbar = NavigationToolbar2Tk(self.canvas_network, self.left_panel)
    ...
    def _clear_map_highlights(self):
        for art in getattr(self, "_map_highlight_lines", []):
            try: art.remove()
            except: pass
        self._map_highlight_lines = []
        for art_name in ("_map_hit_dot", "_map_annot"):
    ...
    def _on_map_click(self, event):
        ...
        # ヒット点：実点＋半透明のハロー（当たり判定の視覚的手掛かり）
        if used_cartopy:
            tr = ax.projection
            # ハロー
            ax.plot(pos[hit][0], pos[hit][1], "o", ms=18,
                    mfc="tab:blue", alpha=0.18, mec="none",
                    transform=tr, zorder=6)
            # 実点
            self._map_hit_dot = ax.plot(pos[hit][0], pos[hit][1], "o", ms=7,
                                        mfc="tab:blue", mec="white", mew=0.8,
                                        transform=tr, zorder=7)[0]
        else:
            ax.plot(pos[hit][0], pos[hit][1], "o", ms=18,
                    mfc="tab:blue", alpha=0.18, mec="none", zorder=6)
            self._map_hit_dot = ax.plot(pos[hit][0], pos[hit][1], "o", ms=7,
                                        mfc="tab:blue", mec="white", mew=0.8, zorder=7)[0]

        # 注釈
        txt = f"{hit}\n({pos[hit][1]:.3f}°N, {pos[hit][0]:.3f}°E)"
        if used_cartopy:
            self._map_annot = ax.annotate(
                txt, xy=pos[hit], xytext=(10, 12), textcoords="offset points",
                bbox=dict(boxstyle="round,pad=0.25", fc="w", ec="#888", alpha=0.95),
                fontsize=9, color="#222", transform=ax.projection, zorder=8
            )
        else:
            self._map_annot = ax.annotate(
                txt, xy=pos[hit], xytext=(10, 12), textcoords="offset points",
                bbox=dict(boxstyle="round,pad=0.25", fc="w", ec="#888", alpha=0.95),
                fontsize=9, color="#222", zorder=8
            )

        # 接続エッジ（大圏で強調）
        lines = []
        def _edge(u, v, color="#ff4444", lw=2.4, z=5):
            if u not in pos or v not in pos:
                return
            x1, y1 = pos[u]; x2, y2 = pos[v]
            if used_cartopy:
                import cartopy.crs as ccrs
                ln, = ax.plot([x1, x2], [y1, y2], color=color, lw=lw, alpha=0.95,
                            transform=ccrs.Geodetic(), zorder=z)
            else:
                ln, = ax.plot([x1, x2], [y1, y2], color=color, lw=lw, alpha=0.95, zorder=z)
            lines.append(ln)

        import networkx as nx
        G = getattr(self, "G", None)
        if isinstance(G, (nx.Graph, nx.DiGraph)) and hit in G:
            if hasattr(G, "out_edges"):  # DiGraph
                for u, v in G.out_edges(hit): _edge(u, v)
                for u, v in G.in_edges(hit):  _edge(u, v)
            else:
                for u, v in G.edges():
                    if u == hit or v == hit: _edge(u, v)
        else:
            for (u, v) in getattr(self, "_map_high_edges", []):
                if u == hit or v == hit: _edge(u, v)

        self._map_highlight_lines = lines
    ...
    def _on_map_scroll(self, event):
        ...
        # 拡大率：上でズームイン、下でズームアウト
        zbase = 1.25
        zoom  = (1.0 / zbase) if getattr(event, "button", "") == "up" else zbase
        ...
        # マウス位置から見た左右・上下の比率（これを保ったまま拡大縮小）
        p_left   = (cx - xmin) / w
        p_right  = 1.0 - p_left
        p_bottom = (cy - ymin) / h
        p_top    = 1.0 - p_bottom
        ...
        xmin2 = cx - p_left   * w2
        xmax2 = cx + p_right  * w2
        ymin2 = cy - p_bottom * h2
        ymax2 = cy + p_top    * h2

        if used_cartopy and data_crs:
            ax.set_extent([xmin2, xmax2, ymin2, ymax2], crs=data_crs)
        else:
    ...
    def _on_map_key(self, event):
        ...
        def _fit(xs, ys, pad_x=10, pad_y=5):
            if not xs or not ys:
                return
            xmin, xmax = min(xs), max(xs)
            ymin, ymax = min(ys), max(ys)
            if used_cartopy and data_crs:
                ax.set_extent([xmin-pad_x, xmax+pad_x, ymin-pad_y, ymax+pad_y], crs=data_crs)
            else:
                ax.set_xlim(xmin-pad_x, xmax+pad_x)
                ax.set_ylim(ymin-pad_y, ymax+pad_y)

        pos = getattr(self, "_map_pos", {})
        if event.key == "a":  # all nodes
            xs = [x for x,_ in pos.values()]; ys=[y for _,y in pos.values()]
            _fit(xs, ys)
        elif event.key == "w":  # world
            if used_cartopy:
                ax.set_global()
            else:
                ax.set_xlim(-180, 180); ax.set_ylim(-90, 90)
        elif event.key == "f":  # selected product nodes にフィット
            # show_world_map で作った highlight_edges を利用して該当ノード集合を拾う
            high = getattr(self, "_map_high_edges", [])
            nodes = set()
            for u,v in high:
    ...
    def show_world_map(self, product_name=None):
        ...
        - 全エッジ：薄灰
        - 選択製品のエッジ：赤（矢印）
        - cartopy があれば太平洋中心の PlateCarree(central_longitude=180)
        でベースマップ＆都市ラベルを描き、エッジは大圏(Geodetic)で描画。
        座標は Node.lon / Node.lat（経緯度）を使用。
        """
        ...
        if ax is None and hasattr(self, "view_nx_matlib4opt"):
            self.view_nx_matlib4opt()
            ax = getattr(self, "ax_network", None)
        if ax is None:
        ...
        # 2) 背景（cartopy：太平洋中心 / 都市ラベル ；無ければ平面）
        used_cartopy = False
        try:
            import cartopy.crs as ccrs
            ...
            ax.add_feature(cfeature.LAKES.with_scale('110m'), edgecolor='k', facecolor='none', linewidth=0.2)
            ax.add_feature(cfeature.RIVERS.with_scale('110m'), edgecolor='#88a', linewidth=0.2)
            ...
            try:
                ...
                for rec in shpreader.Reader(shpfile).records():
                    if rec.attributes.get('SCALERANK', 99) <= 2:
                        lon, lat = rec.geometry.x, rec.geometry.y
                        name = rec.attributes.get('NAME', '')
                        ax.plot(lon, lat, marker='o', ms=1.8, color='#444',
                                transform=ccrs.PlateCarree(), zorder=1)
                        ax.text(lon, lat, f' {name}', fontsize=7, color='#333',
                                transform=ccrs.PlateCarree(), zorder=1)
            except Exception:
                pass
        ...
        # 3) ノード点（lon/lat）
        def _lat(n): return float(getattr(n, "lat", 0.0) or 0.0)
        def _lon(n): return float(getattr(n, "lon", 0.0) or 0.0)

        pos = {}
        hub = {"sales_office", "procurement_office", "supply_point"}
        for name, node in nodes_all.items():
            x, y = _lon(node), _lat(node)
            pos[name] = (x, y)
            color = "#1f77b4" if name not in hub else "#444444"
            ms = 15 if name not in hub else 30
            if used_cartopy:
                import cartopy.crs as ccrs  # for transform
                ax.plot(x, y, "o", color=color, ms=ms/3,
                        transform=ccrs.PlateCarree(), zorder=3)
                ax.text(x, y, f" {name}", fontsize=8, va="bottom",
                        transform=ccrs.PlateCarree(), zorder=3)
            else:
                ax.plot(x, y, "o", color=color, ms=ms/3, zorder=3)
                ax.text(x, y, f" {name}", fontsize=8, va="bottom", zorder=3)
        ...
        if product_name and env and getattr(env, "prod_tree_dict_OT", None):
            ...
            if root:
                for p, c in self._iter_parent_child(root):
                    highlight_edges.add((getattr(p, "name", ""), getattr(c, "name", "")))

        # 5.5) フィット対象ノード名（選択製品のみ）
        selected_names = set()
        if root is not None:
            # PlanNode ツリーを辿って物理ノード名を収集
            for n in self._walk_nodes(root):
                nm = getattr(n, "name", None)
                if nm:
                    selected_names.add(nm)
        if not selected_names and highlight_edges:
            # 念のためエッジ端点からも補完
            for u, v in highlight_edges:
                if u: selected_names.add(u)
                if v: selected_names.add(v)

        # 6) エッジ描画ヘルパ（矢印対応）— ax/pos/used_cartopy 準備後に定義！
        try:
            import cartopy.crs as ccrs  # 既にimport済みなら再利用
        except Exception:
            ccrs = None
        ...
        def _seg(u, v, color, lw, arrow=False, z=3):
            ...
            if used_cartopy and ccrs:
                ax.plot([x1, x2], [y1, y2], color=color, lw=lw, alpha=0.8,
                        transform=ccrs.Geodetic(), zorder=z)  # 大圏
                if arrow:
        ...
        # 7) 描画：全エッジ（灰）→ ハイライト（赤・矢印）
        for (u, v) in all_edges:
            _seg(u, v, "#cccccc", 1.0, arrow=False, z=3)
        for (u, v) in highlight_edges:
            _seg(u, v, "red", 2.0, arrow=True, z=5)

        # 8) 自動フィット（選択製品ノードだけにフィット）
        fit = True
        if hasattr(self, "world_map_fit_var"):
            try:
                fit = bool(self.world_map_fit_var.get())
            except Exception:
                fit = getattr(self, "world_map_fit", True)
        else:
            fit = getattr(self, "world_map_fit", True)

        def _wrap_lon(lon, center=180.0):
            # 太平洋中心（central_longitude=180）に合わせて経度を [-180,180) へ巻き直し
            return ((lon - center + 180.0) % 360.0) - 180.0

        if pos:
            # ★ フィット対象は選択製品のノードだけ。無ければ全体。
            fit_keys = [k for k in selected_names if k in pos] or list(pos.keys())

            xs = [pos[k][0] for k in fit_keys]
            ys = [pos[k][1] for k in fit_keys]

            # 緯度は通常の min/max、経度は 180 基準で wrap してから min/max
            ymin, ymax = min(ys), max(ys)
            ...
            min_lon_width = 10.0
            min_lat_width = 6.0
            if (xmax_c - xmin_c) < min_lon_width:
                cx = 0.5 * (xmin_c + xmax_c)
                xmin_c, xmax_c = cx - min_lon_width/2.0, cx + min_lon_width/2.0
            if (ymax - ymin) < min_lat_width:
                cy = 0.5 * (ymin + ymax)
                ymin, ymax = cy - min_lat_width/2.0, cy + min_lat_width/2.0

            if fit:
                if used_cartopy:
                    ax.set_extent(
                        [xmin_c - lon_pad, xmax_c + lon_pad, ymin - lat_pad, ymax + lat_pad],
                        crs=ax.projection  # PlateCarree(central_longitude=180)
                    )
                else:
        ...
        # 9) 凡例＆再描画
        ax.plot([], [], color="#cccccc", label="All Edges")
        ax.plot([], [], color="red", label=f"{product_name or 'selected'} edges")
        ax.legend(loc="lower left", fontsize=8)
        if hasattr(self, "canvas_network"):
            self.canvas_network.draw_idle()

        # === ここから追加：地図インタラクション用の状態を保存 ===
        # クリック探索に使う
        self._map_ax = ax
        self._map_pos = pos                     # {node_name: (lon, lat)}
        self._map_nodes = nodes_all             # {node_name: Node}
        self._map_all_edges = list(all_edges)   # [(u,v),...]
        self._map_high_edges = list(highlight_edges)
        self._map_used_cartopy = used_cartopy

        try:
            import cartopy.crs as ccrs
            # データ（経緯度）の CRS と、地図の投影（今回=中心180のPlateCarree）を保持
            self._map_data_crs = ccrs.PlateCarree()
            self._map_map_crs  = ax.projection if used_cartopy else None
        except Exception:
            self._map_data_crs = None
            self._map_map_crs  = None

        # ハイライト等の一時アーティストを消すための入れ物
    ...
    def show_world_map(self, product_name=None):
        """
        世界地図ビュー（太平洋中心）。選択製品は赤でハイライト。
        - 点とラベルの transform は「データの CRS = PlateCarree()」に統一
        - エッジは大圏(Geodetic)で描画
        - クリック/ホイール/キーイベントは fig.canvas に毎回つなぎ直す
        """
        # --- Axes / Figure を用意 ---
        self._ensure_network_axes()
        ax = getattr(self, "ax_network", None)
        ...
        # --- 背景（Cartopy: PlateCarree(central_longitude=180)） ---
        ax.clear(); ax.set_title("Global Supply Chain Map", fontsize=12)
        used_cartopy = False
        data_crs = None  # = 緯度経度(0°基準) の CRS
        try:
            import cartopy.crs as ccrs
            import cartopy.feature as cfeature
            from cartopy.io import shapereader as shpreader
            ...
            fig = ax.figure
            ax.remove()
            proj_map = ccrs.PlateCarree(central_longitude=180)  # ★太平洋中心
            ax = fig.add_subplot(111, projection=proj_map)
            self.ax_network = ax

            ax.add_feature(cfeature.OCEAN.with_scale('110m'),  facecolor="#e6f2ff")
            ax.add_feature(cfeature.LAND .with_scale('110m'),  facecolor="#f6f6f6")
            ax.add_feature(cfeature.LAKES .with_scale('110m'), edgecolor='k',
                        facecolor='none', linewidth=0.2)
            ax.add_feature(cfeature.RIVERS.with_scale('110m'), edgecolor='#88a',
                        linewidth=0.2)
            ...
            used_cartopy = True
            data_crs = ccrs.PlateCarree()  # ★データは 0°基準の経緯度
        except Exception:
            ax.set_xlim(-180, 180); ax.set_ylim(-90, 90); ax.set_facecolor("#e6f2ff")

        # --- ノード描画（ハロー＋実点 / transform は data_crs） ---
        def _lat(n): return float(getattr(n, "lat", 0.0) or 0.0)
        def _lon(n): return float(getattr(n, "lon", 0.0) or 0.0)
        ...
        for name, node in nodes_all.items():
            ...
            if used_cartopy:
                ax.plot(x, y, "o", ms=max(ms, 12), mfc=color, alpha=0.15,
                        mec="none", transform=data_crs, zorder=3)      # ハロー
                ax.plot(x, y, "o", ms=7, mfc=color, mec="white", mew=0.8,
                        transform=data_crs, zorder=4)                   # 本体
                ax.text(x, y, f" {name}", fontsize=8, va="bottom",
        ...
        # --- ハイライト（選択品目） ---
        highlight_edges, selected_names = set(), set()
        root = None
        if product_name and env and getattr(env, "prod_tree_dict_OT", None):
            root = env.prod_tree_dict_OT.get(product_name)
            if root:
        ...
        if pos:
            ...
            if fit:
                if used_cartopy:
                    ax.set_extent([xmin_c - lon_pad, xmax_c + lon_pad,
        ...
        # イベントは fig.canvas に接続（こちらが確実）
        canvas = fig.canvas                      # << 重要：Figure側の canvas
        ...
        # Tk 側にフォーカスを明示してホイールイベントを確実に受ける
        try:
    ...
    def show_world_map_1before(self, product_name=None):
        """
        左パネルを世界地図表示に切替え。
        - 全エッジ：薄灰
        - 選択製品のエッジ：赤（矢印）
        - cartopy があれば太平洋中心の PlateCarree(central_longitude=180)
        でベースマップ＆都市ラベルを描き、エッジは大圏(Geodetic)で描画。
        座標は Node.lon / Node.lat（通常の経緯度）を使用。
        """
        # Axes を確保
        self._ensure_network_axes()
        ax = getattr(self, "ax_network", None)
        ...
        try:
            ...
            used_cartopy = True
            data_crs = ccrs.PlateCarree()   # ★ データは通常の経緯度
        except Exception:
            ax.set_xlim(-180, 180); ax.set_ylim(-90, 90)
            ax.set_facecolor("#e6f2ff")
        ...
        for name, node in nodes_all.items():
            ...
            if used_cartopy:
                # ★ transform は“データの CRS”（=通常の PlateCarree）にする
                ax.plot(x, y, "o", ms=max(ms, 12), mfc=color, alpha=0.15,
                        mec="none", transform=data_crs, zorder=3)             # ハロー
                ax.plot(x, y, "o", ms=7, mfc=color, mec="white", mew=0.8,
                        transform=data_crs, zorder=4)                          # 実点
                ax.text(x, y, f" {name}", fontsize=8, va="bottom",
                        transform=data_crs, zorder=4)
            else:
        ...
        def _seg(u, v, color, lw, arrow=False, z=3):
            ...
            if used_cartopy and ccrs:
                ...
                if arrow:
                    ax.plot(x2, y2, marker='>', color=color, ms=6,
                            transform=data_crs, zorder=z+1)   # ★ 終点もデータ CRS
            else:
                ax.plot([x1, x2], [y1, y2], color=color, lw=lw, alpha=0.8, zorder=z)
                if arrow:
                    ax.plot(x2, y2, marker='>', color=color, ms=6, zorder=z+1)
        ...
        if hasattr(self, "world_map_fit_var"):
            try: fit = bool(self.world_map_fit_var.get())
            except Exception: fit = getattr(self, "world_map_fit", True)
        else:
            fit = getattr(self, "world_map_fit", True)
        ...
        if pos:
            ...
            min_lat_width = 6.0
            if (xmax_c - xmin_c) < min_lon_width:
                cx = 0.5 * (xmin_c + xmax_c); xmin_c, xmax_c = cx - 5.0, cx + 5.0
            if (ymax - ymin) < min_lat_width:
                cy = 0.5 * (ymin + ymax); ymin, ymax = cy - 3.0, cy + 3.0

            if fit:
                if used_cartopy:
                    ax.set_extent([xmin_c - lon_pad, xmax_c + lon_pad,
                                ymin - lat_pad,   ymax + lat_pad],
                                crs=ax.projection)  # 180°中心座標で OK
                else:
                    ax.set_xlim(xmin_c - lon_pad, xmax_c + lon_pad)
                    ax.set_ylim(ymin   - lat_pad, ymax   + lat_pad)
            else:
        ...
        # --- 状態保存＆イベント再接続 ---
        # 古い接続を解除
        if hasattr(self, "_map_cids"):
            try:
                for cid in self._map_cids:
                    self.canvas_network.mpl_disconnect(cid)
            except Exception:
                pass
        ...
        self._map_high_edges   = list(highlight_edges)
        self._map_used_cartopy = used_cartopy
        self._map_data_crs     = data_crs  # PlateCarree(0°) or None

        # 一時アーティストクリア
        self._map_annot = None
        self._map_hit_dot = None
        self._map_highlight_lines = []

        # イベント接続
        self._map_cids = [
            self.canvas_network.mpl_connect("scroll_event",       self._on_map_scroll),
            self.canvas_network.mpl_connect("button_press_event", self._on_map_click),
            self.canvas_network.mpl_connect("key_press_event",    self._on_map_key),
        ]
    ...
    def _event_lonlat(self, event):
        """マウスイベント -> (lon, lat)。Cartopy でも平面でもOK。"""
        ax  = getattr(self, "_map_ax", None)
        ...
        if used_cartopy:
            try:
                import cartopy.crs as ccrs
                # event.xdata/ydata は表示座標系。PlateCarree に変換
                tr = ax.transData + ccrs.PlateCarree()._as_mpl_transform(ax)
                inv = tr.inverted()
...
#".format(total_revenue, total_profit))
    ...
    def on_network_click_OLD(self, event):
        ...
        select = self.nodes_prod_outbound.get(closest) or self.nodes_prod_inbound.get(closest)
        if select is None:
            return
    ...
    def on_network_click(self, event):
        ...
        # ★ dict固定の製品別辞書を参照（list化バグを回避）
        select = None
        if isinstance(getattr(self, "nodes_prod_outbound", None), dict):
            select = self.nodes_prod_outbound.get(closest)
        if select is None and isinstance(getattr(self, "nodes_prod_inbound", None), dict):
            select = self.nodes_prod_inbound.get(closest)
        if select is None:
            print("Error: Node not found or value is None")
            return
        ...
        ax = self.ax_network
        if getattr(self, "annotation_artist", None):
            self.annotation_artist.remove()
        self.annotation_artist = ax.annotate(
            txt,
            xy=self.pos_E2E[closest],
            xytext=(0.5, 0.02), textcoords='axes fraction',
            ha='center', color='red', fontsize=9,
