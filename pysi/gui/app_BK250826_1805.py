...
class PSIPlannerApp:
    ...
    def setup_ui_OLD250826(self):
        ...
        # Viewメニュー
        view_menu.add_command(label="Network Graph",
                            command=lambda: (setattr(self, "view_mode", "network"),
        ...
        #@250826 ADD
        # --- Select Product パネル ---
        frm = ttk.LabelFrame(self.frame, text="Select Product")   # ← right_panel → self.frame
        frm.pack(fill="x", padx=4, pady=2)

        self.product_name_list = sorted(
            (getattr(self.psi, "prod_tree_dict_OT", {}) or {}).keys()
        )
        self.cb_product = ttk.Combobox(
            frm, values=self.product_name_list, state="readonly", width=18
        )
        ...
        # ここで必ずネットワークAxes/Canvasを用意
        self._ensure_network_axes()
        # ★ PSI描画領域も先に用意
        self._ensure_psi_area()
        # 初回描画（set()/current() ではイベント発火しないので明示呼び出し）
        ...
        #既定値（DADxxx）を強制したい場合のワンライナー
        #もし “必ず DADxxx を最初に使いたい” なら、setup_ui() の初期化直後に：
        ## setup_ui() の末尾に、初回だけ強制したいとき（任意）
        #if not getattr(self, "decouple_node_selected", None):
        #    self.decouple_node_selected = self._detect_default_decouple_nodes(
        #        getattr(self, "product_selected", None)
        #    )
    ...
    def setup_ui(self):
        ...
        self.left_panel  = ttk.Frame(self.main_panel)   # ネットワーク（または地図）専用
        self.right_panel = ttk.Frame(self.main_panel)   # セレクタ/パラメータ/PSI 専用
        self.left_panel.pack(side="left", fill="both", expand=True)
        self.right_panel.pack(side="right", fill="y")

        # 互換のため、従来の self.frame を右パネルに割り当て
        self.frame = self.right_panel

        # =====================================================================
        # 4) Select Product コンボは右パネルに入れる
        # =====================================================================
        frm = ttk.LabelFrame(self.right_panel, text="Select Product")
        frm.pack(fill="x", padx=4, pady=2)
        ...
        # 右＝PSI スクロール領域を用意
        self._ensure_psi_area(self.right_panel)
...
# …が意外とデリケートなポイント
    ...
    def show_world_map(self, product_name=None):
        ...
        try:
            ...
            # --- 主要都市（Populated Places simple） ---
            try:
                shpfile = shpreader.natural_earth(resolution='110m', category='cultural',
                                                name='populated_places_simple')
                for rec in shpreader.Reader(shpfile).records():
                    if rec.attributes.get('SCALERANK', 99) <= 2:  # 主要都市のみ
                        lon, lat = rec.geometry.x, rec.geometry.y
                        name = rec.attributes.get('NAME', '')
                        ax.plot(lon, lat, marker='o', ms=1.8, color='#444', transform=ccrs.PlateCarree(), zorder=2)
                        ax.text(lon, lat, f' {name}', fontsize=7, color='#333', transform=ccrs.PlateCarree(), zorder=2)
            except Exception:
                pass  # 都市レイヤが取得できない場合は黙ってスキップ

            ax.set_global()
            used_cartopy = True
        except Exception:
            # フォールバック（matplotlib平面）
            ax.set_xlim(-180, 180)
            ax.set_ylim(-90, 90)
        ...
        for name, node in nodes_all.items():
            ...
            if used_cartopy:
                ax.plot(x, y, "o", color=color, ms=ms/3, transform=ax.projection, zorder=3)
                ax.text(x, y, f" {name}", fontsize=8, va="bottom", transform=ax.projection, zorder=3)
            else:
