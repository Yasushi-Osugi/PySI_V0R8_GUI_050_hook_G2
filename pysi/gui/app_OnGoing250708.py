    ...
    def load_data_files(self):
        ...
        if directory:
            ...
            def read_p_month_data_csv(filepath, target_node, target_product, target_year):
                ...
                print("read_p_month_data_csv(filepath, target_node, target_product, target_year):", filepath, target_node, target_product, target_year)
...
# **********************************
            ...
            # =============================================
            # Step 1: P_month_data.csv の読み込みと週次変換
            # =============================================
            if "P_month_data.csv" in data_file_list:
                ...
                print(f"[INFO] Loaded P_month_data.csv → psi4supply populated")

            # =============================================
            # Step 2: perform_allocation による S→P 割当
            # =============================================
            for node_name, node in self.nodes_outbound.items():

                # 【1】psi4demand[w][0] から S_lot を収集
                demand_map = {
                    w: node.psi4demand[w][0]
                    for w in range(len(node.psi4demand))
                    if node.psi4demand[w][0]
                }

                # 【2】供給キャパシティ（週ごとの P_lot 数を参照）
