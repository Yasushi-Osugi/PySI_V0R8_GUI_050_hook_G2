...
#@250630 STOP GO
    def load_data_files(self):
        ...
        if directory:
            ...
            # **************************************
            # make slicing subtree by product_name
            # **************************************

            subtree_inbound_list = {} # {product_name:subtree, ,,,}
            subtree_outbound_list = {} # {product_name:subtree, ,,,}
            
            # subtree rule
            # 1. by product_nameのsubtreeである。
            # 2. "supply_point"をtop_rootに持つ
            # 3. sub_nodes = subtree_node_dictで生成する
            # 4. subtree_list = {} # {product_name:sub_nodes, ,,,}に格納する
            # 5. 上記より、subtree_root = sub_nodes["supply_point"]で取り出せる

            product_name_list = list(node_dict["supply_point"].sku_dict.keys())
            print("product_name_list", product_name_list)

            sub_nodes = {} # slicing tree node"s"
            ...
            for prod_nm in product_name_list:
                ...
                subtree_inbound = extract_subtree_by_product(root_node_inbound, prod_nm)
                #subtree_inbound, sub_nodes4in = extract_subtree_by_product(root_node_inbound, prod_nm, sub_nodes)

                subtree_outbound = extract_subtree_by_product(root_node_outbound, prod_nm)
                #subtree_outbound, sub_nodes4out = extract_subtree_by_product(root_node_outbound, prod_nm, sub_nodes)

                subtree_inbound_list[prod_nm] = subtree_inbound
                subtree_outbound_list[prod_nm] = subtree_outbound
            ...
            for prod_nm in product_name_list:
                ...
                if subtree_inbound:
                    ...
                    subtree_inbound_list[prod_nm].print_tree()
            
                if subtree_outbound:
                    print("Outbound subtree:")
                    subtree_outbound_list[prod_nm].print_tree()




            # **************************************
            # slicing subtree test 
            # **************************************

            # 例えば tree["root_node_inbound"] から productA の部分ツリーを抽出する場合
            #@250713 STOP
...
# **** A PART of ORIGINAL load_data_files *****
        ...
        #@250717 ADD for viisualize test
        # network area
        ...
        ## 例：ルートSKU定義
        #root_val_dict = {
        #    "RICE1": root_node.get_sku("RICE1"),
        #    "RICE2": root_node.get_sku("RICE2"),
        #}
        root_val_dict = {}

        product_name_list = list( node_dict["supply_point"].sku_dict.keys() )

        print("node_dict[ supply_point ].sku_dict.keys", product_name_list )

        for prod_nm in product_name_list:

            #print("product_nane at supply_point", product_name)


            #@250716 MEMO
            # 用意した製品別subtree_list{product_name:subtree,,,}で処理
            # subtree_list[product_name] => sub_nodesを持っておく?


            def find_sku_with_cost_standard_flag(prod_nm, nodes, flag_value):

                for node_name, node in nodes.items():

                    sku = node.sku_dict.get(prod_nm)

                    if sku is None:
                        continue  # 該当SKUがないノードはスキップ
            
                    print("node_name prod_nm sku.cost_standard_flag flag_value", node_name, prod_nm, sku.cost_standard_flag, flag_value)
            
                    if sku.cost_standard_flag == flag_value:
            ...
            # 販売基準価格の設定
            # ********************************
            def find_sku_with_cost_standard_flag_OLD(prod_nm, nodes,flag_value):

                for node_name, node in nodes.items():

                    print("node_name prod_nm node.cost_standard_flag flag_value", node_name, prod_nm, node.cost_standard_flag, flag_value )
    
                    if node.sku_dict[prod_nm].cost_standard_flag == flag_value:

                    #@STOP
                    #if ( node.sku_dict[prod_nm].cost_standard_flag == flag_value ) and ( node.product_name == prod_nm ):
            ...
            #
            #    for node_name, node in nodes.items():
            #        print("node_name node.name node.cost_standard_flag flag_value", node_name, node.name, node.cost_standard_flag, flag_value )
            #
            #        if ( node.cost_standard_flag == flag_value ) and ( node.product_name == prod_nm ):
            #            return node_name, node
            #
            #    return None, None
        ...
        #node_name, base_leaf = find_node_with_cost_standard_flag(nodes_outbound, 100)
            ...
            leaf_node_name, base_leaf_node = find_sku_with_cost_standard_flag(prod_nm, nodes_outbound, 100)

            self.base_leaf_name[ prod_nm ] = leaf_node_name

            if leaf_node_name is None:
                print("NO cost_standard = 100 in profile")
            else:
                print(f"Node name: {leaf_node_name}, Base leaf: {base_leaf_node}")

            #@250715 STOP
            #sku = base_leaf_node.sku_dict[ prod_nm ]
            #leaf_sku = base_leaf_node.get_sku( prod_nm )


        #@250715 TODO MEMO
        # 0. product_name_list = nodes_outbound["supply_point"].sku_dict.keys
        # 1. for product_name in product_name_list:
        # 1. subtree_list[product_name] = make_sliced_subtree_by_product(...)
        # 1. 親も指すnode.parentをセットする

        # 2. for subtree_root in subtree_list
        # 2. root_price = set_price_leaf2root(base_leaf, subtree_root, 100)

        # 3. sku = nodes_outbiund[node_name].sku_dict[product_name]
        ...
        #set_value_chain_outbound(root_price, self.root_node_outbound)
            ...
            #@250712 ADD UPDATE
            # RICE1 の末端 SKU からルート SKU まで伝播

            leaf_sku = base_leaf_node.get_sku( prod_nm )


            root_sku = node_dict["supply_point"].get_sku( prod_nm )
            #root_sku = root_node.get_sku( prod_nm )
        

            #@250716 MEMO ここで、by SKUの判定をしなくても、
            # by product_namrのsub_treeがsliceされているので、
            # 通常のnode travelingでset_price_leaf2root()してskuにsetできる

            root_price = set_price_leaf2root_sku(
                sku=leaf_sku,
                root_sku=root_sku,
                val=100.0, # val=leaf_price,
                node_dict=node_dict  # nodes
            )

            #root_price = set_price_leaf2root_sku(leaf_sku, root_sku, val=100.0, nodes )

            print("root_price", root_price)


            root_val_dict[ prod_nm ] = root_sku # "supply_point"

            root_val_dict[ prod_nm ].cs_price_sales_shipped = root_price
        ...
        #@250712 ADD STOP
        ## RICE1 の末端 SKU からルート SKU まで伝播
        #leaf_sku = leaf_node.get_sku("RICE1")
        #root_sku = root_node.get_sku("RICE1")
        #
        #set_price_leaf2root_sku(leaf_sku, root_sku, val=100.0)
        #
        #print("root_price", root_price)
        ...
        #    "RICE2": root_node.get_sku("RICE2"),
        #}
        ## ルートSKUにcs_price_sales_shippedだけ事前に入れておく
        #root_val_dict["RICE1"].cs_price_sales_shipped = 100.0
        #root_val_dict["RICE2"].cs_price_sales_shipped = 110.0
        #
        ## 再帰実行
        #set_value_chain_outbound_sku(root_val_dict, root_node)
        ...
        #}
        #
        #root_val_dict = run_price_propagation_scenario(
        #    product_price_map=product_price_map,
        #    leaf_node=node_dict["GR_CS_JPN"],
        #    root_node=node_dict["DADKosihikari"],
        #    node_dict=node_dict
        #)


#出力結果（使い道）
#root_val_dict["RICE1"].cs_price_sales_shipped → 価格ベース
#
#root_val_dict["RICE1"].cs_profit_accume → 累積利益
#
#下流ノードのSKUにも cs_〜 項目が評価済み
#
#これにより、SKUレベルでのCS（Cost Structure）とProfitシミュレーションが完了し、
#この後は次のような分析が可能になります：
#
#この後にできる分析
#分析テーマ	方法／対象
#SKUごとの累積利益可視化	sku.cs_profit_accume
#原材料比率や利益率のヒートマップ	sku.cs_direct_materials_costs / cs_price#_sales_shipped
#ノード別利益貢献ランキング	sku.cs_profit の合計をノード単位で集計
#価格変更影響シミュレーション	product_price_map を変えてシナリオ比較
