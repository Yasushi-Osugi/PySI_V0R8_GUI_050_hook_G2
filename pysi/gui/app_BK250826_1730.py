...
class PSIPlannerApp:
    ...
    def _ensure_network_axes(self):
        from matplotlib.figure import Figure
        from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
        if getattr(self, "ax_network", None) is not None and getattr(self, "canvas_network", None) is not None:
            return
        # 左ペイン（なければ self.frame にフォールバック）
        parent = getattr(self, "left_panel", None) or getattr(self, "left_frame", None) or getattr(self, "frame", None)
        if parent is None:
            # 最後の手段：root直下
            parent = self.root

        self.fig_network = Figure(figsize=(5, 4), dpi=100)
        self.ax_network = self.fig_network.add_subplot(111)
        self.canvas_network = FigureCanvasTkAgg(self.fig_network, master=parent)
        self.canvas_network.draw()
        self.canvas_network.get_tk_widget().pack(fill=tk.BOTH, expand=True)


    # クラス PSIPlannerApp 内に追加
    def _ensure_psi_area(self):
        """
        PSIグラフを描くスクロール領域（self.canvas_psi / self.scrollable_frame）を
        まだ作っていなければ作る。既にあれば何もしない。
        """
        # 既存があればそのまま
        ...
        # 親コンテナの候補（右パネルがあれば優先、なければ self.frame、最後の砦は root）
        parent = getattr(self, "right_panel", None) \
            or getattr(self, "frame", None) \
            or getattr(self, "root", None)

        # ラベルフレーム（右側に載せる想定。環境に合わせて pack 方向は調整可）
        psi_frame = ttk.LabelFrame(parent, text="PSI Graph")
        psi_frame.pack(side="right", fill="both", expand=True, padx=4, pady=2)
