...
class PSIPlannerApp:
    def __init__(self, root, config):
        self.root = root
        self.root.title("Global Weekly PSI Planner")
        self.config = config # main.pyから渡されるconfigオブジェクトを使用

        # ダミーのメソッド（実際のアプリケーションでは実装されているはずです）
        # setup_ui内で参照されるすべてのメソッドをここで初期化します
        ...
        # MatplotlibのFigureとAxesを初期化
        # setup_uiの前に初期化しておくことで、setup_ui内で参照できるようになります
        self.fig_network, self.ax_network = plt.subplots(figsize=(6, 8))
        self.canvas_network = None # setup_uiで実際のTkinterウィジェットに紐付けられます

        # PSIグラフ用のCanvasとScrollableFrameも初期化
        self.canvas_psi = None
...
# +++++++++++++++++++++++++++++++++++++++++++
    ...
    def setup_ui(self):
        ...
        # self.fig_network, self.ax_network は __init__ で既に初期化済み
...
#@250630 STOP GO
    def load_data_files(self):
        ...
        if directory:
            ...
            def generate_iso_calendar(plan_year, plan_weeks):
...
# **************************************
#    plan_year = 2025
#    month2cap = read_p_month_data_csv(
#        'P_month_data.csv', 'DADJPN', 'prod-A', plan_year
#    )
#    start, count = generate_iso_calendar(plan_year)
#    pweek = month_to_week_buckets(month2cap, start, count)
#    harvest_weeks = week_data_to_capacities(pweek, lot_size=1)
#    print("harvest_weeks =", harvest_weeks)
# **************************************
                        ...
                        plan_weeks = self.plan_range*53
                        print("self.plan_range =", self.plan_range)
                        print("self.plan_range*53 = plan_weeks =", plan_weeks)
                        start, count = generate_iso_calendar(self.plan_year_st, plan_weeks)
                        ...
                        print("node_name dad_node.name", node_name, dad_node.name )
                        ...
                        demand_map = {
                            w: psi4demand[w][0]
                            for w in range(len(psi4demand))
                            if len(psi4demand[w][0]) > 0
                        }


                        print("demand_map", demand_map)
                        ...
                        print( "BEFORE ALLOC dad_node.psi4supply", dad_node.psi4supply )


                        # 本当にdeepcopyでコピー
                        #for w in range(1, self.plan_range + 1):

                        #    print("before dad_node.psi4supply[w][3]",w, dad_node.psi4supply[w][3])
                        #    dad_node.psi4supply[w][3] = copy.deepcopy(psi4supply[w][3])
                        #    print("after dad_node.psi4supply[w][3] psi4supply",dad_node.psi4supply[w][3], psi4supply[w][3])
                        ...
                        #        #dad_node.psi4supply[w][3] = list(psi4supply[w][3]) 
...
# *************************
    ...
    def show_psi_OLD2(self, bound, layer):
        print("making PSI graph data...")
