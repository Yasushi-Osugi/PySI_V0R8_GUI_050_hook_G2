...
# [最終修正] _on_map_key を「緯度優先フィット」ロジックに対応
    ...
    def _on_map_key(self, event):
        ...
        if event.key == "a":
            if pos:
                _fit_lonlat([p[0] for p in pos.values()], [p[1] for p in pos.values()])
        elif event.key == "w":
            ...
            if nodes:
                lons = [pos[n][0] for n in nodes if n in pos]
                lats = [pos[n][1] for n in nodes if n in pos]
                if lons and lats: _fit_lonlat(lons, lats)
...
# [修正] ======== 水平フォーカス調整（5%）に対応したヘルパー関数 ========
    ...
    def _fit_lonlat_stop(lons, lats, edges=None):
        """
        緯度（縦）を枠いっぱいにフィット。
        水平のフォーカスは:
        ...
        def min_east_positive(lon_list):
            ...
            arr360 = (arr + 360.0) % 360.0  # 東経を 0..360° に正規化
            i = int(np.argmin(arr360))      # 最小のもの＝一番左にある東経
            return float(arr[i])
        ...
        if not np.isfinite(lat_min) or not np.isfinite(lat_max):
            return
        if np.isclose(lat_min, lat_max):   # 点/線だけのときの下限
            lat_max = lat_min + 1.0
        # 極の特異を避ける
        lat_min = max(-89.0, lat_min); lat_max = min(89.0, lat_max)
        ...
        data_crs = getattr(self, "_map_data_crs", ccrs.PlateCarree())  # 標準 lon/lat(0°)
        map_crs  = ax.projection                                           # 画面の投影

        # --- 4) 投影座標で「縦にフィット→横幅算出→左 5% に寄せる」 --------------
        # フォーカス経度上での緯度端点を投影座標へ
        ...
        if event.key == "a":
            if pos:
                ...
                _fit_lonlat(all_lons, all_lats, edges=None)
        elif event.key == "w":
            if used_cartopy: ax.set_global()
            else: ax.set_xlim(-180, 180); ax.set_ylim(-90, 90)
            ...
            if nodes:
                ...
                if lons and lats:
                    ...
                    _fit_lonlat(lons, lats, edges=highlight_edges)


        #elif event.key == "f":
        #    nodes = {u for u, v in highlight_edges} | {v for u, v in highlight_edges}
        #    if nodes:
        #        lons = [pos[n][0] for n in nodes if n in pos]
        #        lats = [pos[n][1] for n in nodes if n in pos]
        #        if lons and lats:
        #            _fit_lonlat(lons, lats, edges=highlight_edges)  # ← edges を渡す
