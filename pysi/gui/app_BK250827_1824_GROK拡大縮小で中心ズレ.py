...
# …が意外とデリケートなポイント
    ...
    def _install_map_interactions(self):
        """右ドラッグのパンだけイベント登録（scroll/key/click は show_world_map で登録済）"""
        if not hasattr(self, "canvas_network") or self.canvas_network is None:
            return

        # 既存接続を解除（この関数で追加したものだけ）
        if hasattr(self, "_map_pan_cids") and self._map_pan_cids:
            try:
                for cid in self._map_pan_cids:
        ...
        c = self.canvas_network
        self._map_pan_cids = [
            c.mpl_connect("button_press_event",  self._on_map_button),
            c.mpl_connect("motion_notify_event", self._on_map_motion),
            c.mpl_connect("button_release_event",self._on_map_release),
        ]
    ...
    def _on_map_scroll(self, event):
        """マウスホイール：カーソル位置を中心にズーム（lon/lat + 画面比で安定化）"""
        ax = getattr(self, "_map_ax", None)
        if ax is None or event.inaxes is not ax:
            return
        ...
        # ---- Cartopy GeoAxes：lon/lat で計算（画面比×cos(lat) を考慮）----
        import cartopy.crs as ccrs, numpy as np, math

        data_crs = getattr(self, "_map_data_crs", ccrs.PlateCarree())
        map_crs  = ax.projection

        # ピクセル縦横比（Axes の見た目の比率）
        bbox = ax.get_window_extent()
        pix_w = max(1.0, bbox.width)
        pix_h = max(1.0, bbox.height)
        pix_ratio = pix_h / pix_w  # 高さ/幅

        # ピボット（lon/lat）
        if event.xdata is None or event.ydata is None:
            pivot = getattr(self, "_map_last_pivot_lonlat", None)
            if pivot is None:
                return
            lon0, lat0 = pivot
        else:
            ...
            self._map_last_pivot_lonlat = (lon0, lat0)
        ...
        # 画面比と緯度の歪み（≈ cos(lat)）に合わせて、緯度スパンを“欲しい値”に補正
        k = max(0.1, math.cos(math.radians(lat0)))   # 0 に近づきすぎるのを防ぐ
        desired_lat_span = lon_span * pix_ratio * k

        # scale を掛けて次のスパンを決定
        lon_span_new = lon_span * scale
        lat_span_new = desired_lat_span * scale

        # 新しい extent（lon/lat）
        xmin_new = lon0 - lon_span_new/2
        xmax_new = lon0 + lon_span_new/2
        ymin_new = max(-89.9, lat0 - lat_span_new/2)
        ymax_new = min( 89.9, lat0 + lat_span_new/2)
    ...
    def _on_map_scroll(self, event):
        ...
        lat_span = (ymax - ymin)
        # 画面比と緯度の歪み（≈ cos(lat)）に合わせて、緯度スパンを“欲しい値”に補正
        desired_lat_span = lon_span * pix_ratio  # k を削除してシンプルにピクセル比で調整
        # scale を掛けて次のスパンを決定
        lon_span_new = lon_span * scale
        lat_span_new = desired_lat_span * scale
        # 新しい extent（lon/lat）
    ...
    def _on_map_button(self, event):
        ...
        if event.button == 3 and event.xdata is not None and event.ydata is not None:
            ...
            # ピボットも更新（直後のホイールでも使える）
            self._map_last_pivot_lonlat = getattr(self, "_map_last_pivot_lonlat", None)
            return
        # 左クリックは show_world_map() で登録した _on_map_click が処理
    ...
    def show_world_map_LOD(self, product_name=None): # LOD（110m/50m/10m 切替）初期セットが重い
        """
        世界地図ビュー（太平洋中心）。選択製品は赤でハイライト。
        - 点/ラベルの transform は「データの CRS = PlateCarree()（0°基準の経緯度）」に統一
        - エッジは大圏(Geodetic)で描画
        ...
        try:
            ...
            # =================================================
            # --- ベースマップ LOD レイヤをまとめて作る（後で可視性だけ切替） ---
            self._map_feats = {}           # {"110m": {name:artist,...}, ...}
            self._map_feat_scale = "110m"  # 現在表示中

            def _add_feat_group(scale: str):
                grp = {}
                grp["OCEAN"]   = ax.add_feature(cfeature.OCEAN.with_scale(scale),
                                                facecolor="#e6f2ff", zorder=0)
                grp["LAND"]    = ax.add_feature(cfeature.LAND.with_scale(scale),
                                                facecolor="#f6f6f6", zorder=0)
                grp["LAKES"]   = ax.add_feature(cfeature.LAKES.with_scale(scale),
                                                edgecolor='k', facecolor='none', linewidth=0.2, zorder=1)
                grp["RIVERS"]  = ax.add_feature(cfeature.RIVERS.with_scale(scale),
                                                edgecolor='#88a', linewidth=0.2, zorder=1)
                grp["COAST"]   = ax.add_feature(cfeature.COASTLINE.with_scale(scale),
                                                linewidth=0.4, edgecolor="#555", zorder=2)
                grp["BORDERS"] = ax.add_feature(cfeature.BORDERS.with_scale(scale),
                                                linewidth=0.4, edgecolor="#777", zorder=2)
                self._map_feats[scale] = grp

            # 3段階作っておく
            for sc in ("110m", "50m", "10m"):
                _add_feat_group(sc)

            # まずは 110m のみ表示
            for sc, grp in self._map_feats.items():
                vis = (sc == "110m")
                for art in grp.values():
                    art.set_visible(vis)
            self._map_feat_scale = "110m"
        ...
        for name, node in nodes_all.items():
            ...
            if used_cartopy:
                ax.plot(x, y, "o", ms=max(ms, 12), mfc=color, alpha=0.15,
                        mec="none", transform=data_crs, zorder=3)      # 半透明ハロー
        ...
        # イベントは Figure 側の canvas に接続（こちらが確実）
        canvas = fig.canvas
        ...
        # ===================================================
        def _set_basemap_lod_from_extent():
            if not getattr(self, "_map_used_cartopy", False):
                return
            import cartopy.crs as ccrs
            ax = self._map_ax
            xmin, xmax, _, _ = ax.get_extent(crs=ccrs.PlateCarree())
            span = abs(xmax - xmin)  # 表示中の経度幅（度）

            # だいたいの目安：広域=110m, 地域=50m, 近接=10m
            if span > 60:
                target = "110m"
            elif span > 10:
                target = "50m"
            ...
            if target != getattr(self, "_map_feat_scale", None):
                for sc, grp in self._map_feats.items():
                    vis = (sc == target)
                    for art in grp.values():
                        art.set_visible(vis)
                self._map_feat_scale = target
                if hasattr(self, "canvas_network"):
                    self.canvas_network.draw_idle()

        # 初期フィット直後にも一度適用
        _set_basemap_lod_from_extent()
        self._set_basemap_lod_from_extent = _set_basemap_lod_from_extent  # 他のハンドラから呼べるように
        # ===================================================
...
# ホイール：カーソル位置を中心にズーム
# Esc：注釈/ハイライトをクリア
# たまにキーが効かない時は、地図を一度クリックして フォーカス を取る（show_world_map() 末尾の focus_set() は入っているので通常は不要）
