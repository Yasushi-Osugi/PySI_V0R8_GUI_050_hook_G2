...
# …が意外とデリケートなポイント
    ...
    def _install_map_interactions(self):
        """地図のスクロール/クリック/パン/キー操作のイベント接続を1回だけやり直す。"""
        # キャンバスが無ければ何もしない
        if not hasattr(self, "canvas_network") or self.canvas_network is None:
            return

        # 既存接続を解除
        if hasattr(self, "_map_cids") and self._map_cids:
            try:
        ...
        self._map_last_pivot_map = None

        # 必須ハンドラの存在を軽くガード（無ければ何もしない）
        for name in ("_on_map_scroll","_on_map_button","_on_map_motion",
                    "_on_map_release","_on_map_key"):
            if not hasattr(self, name):
                return  # 未実装なら静かに撤退

        c = self.canvas_network
        self._map_cids = [
            c.mpl_connect("scroll_event",        self._on_map_scroll),   # ホイールズーム
            c.mpl_connect("button_press_event",  self._on_map_button),   # 右押下=パン開始 / 左押下=注釈
            c.mpl_connect("motion_notify_event", self._on_map_motion),   # パン移動
            c.mpl_connect("button_release_event",self._on_map_release),  # パン終了
            c.mpl_connect("key_press_event",     self._on_map_key),      # f/a/w/esc
        ]
    ...
    def _on_map_scroll(self, event):
        """マウスホイール：カーソル位置を中心にズーム（投影系で一貫処理）"""
        ax = getattr(self, "_map_ax", None)
        if ax is None or event.inaxes is not ax:
            return
        ...
        if used_cartopy:
            # ---- GeoAxes: 投影座標（map_crs）で完結させる ----
            map_crs = ax.projection

            # ピボット（カーソル位置）。xdata/ydata が None の場合は直近の値を使用
            if event.xdata is None or event.ydata is None:
            ...
            # ★ ピボットを中心に新しい範囲を作る（投影座標のまま）
            xmin_new = cx - width  / 2.0
            xmax_new = cx + width  / 2.0
    ...
    def _on_map_key(self, event):
        ...
        def _fit(xs, ys, pad_x=10, pad_y=5):
            ...
            if used_cartopy and data_crs:
                ax.set_extent([xmin - pad_x, xmax + pad_x,
                            ymin - pad_y, ymax + pad_y], crs=data_crs)
            else:
    ...
    def _on_map_button(self, event):
        ...
        if event.button == 3:
            ...
            # 念のためピボットも更新（直後のホイールにも効く）
            self._map_last_pivot_map = self._map_pan_last
            return

        # それ以外（左クリックなど）は “ノード注釈” の既存処理へ
        try:
