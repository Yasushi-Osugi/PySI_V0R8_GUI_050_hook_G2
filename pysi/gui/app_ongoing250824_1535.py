...
class PSIPlannerApp:
    ...
    def _bind_env_to_gui_OLD(self, psi_env):
        self.psi = psi_env
        self.prod_tree_dict_OT = dict(getattr(self.psi, "prod_tree_dict_OT", {}))
        self.prod_tree_dict_IN = dict(getattr(self.psi, "prod_tree_dict_IN", {}))

        # ★ INが空ならOTをフォールバック（KeyError防止）
        if not self.prod_tree_dict_IN:
            self.prod_tree_dict_IN = {k: v for k, v in self.prod_tree_dict_OT.items()}

        self.product_name_list = list(getattr(self.psi, "product_name_list", []))
        ...
        if self.product_name_list:
            ...
            self._apply_selected_product(self.product_selected)  # ← root/out/inセット
            self.view_nx_matlib4opt()
            self.show_psi_by_product("outbound", "demand", self.product_selected)
        else:
            print("[WARN] product_name_list is empty.")


        # 例）product_name に応じたノード辞書を明示的に生成

        if hasattr(psi_env, "prod_tree_dict_OT"):
            root = psi_env.prod_tree_dict_OT.get(self.product_selected)
            if root:
                self.root_node_outbound = root
                self.nodes_prod_outbound = {n.name: n for n in self._walk_nodes(root)}

        if hasattr(psi_env, "prod_tree_dict_IN"):
            root = psi_env.prod_tree_dict_IN.get(self.product_selected)
            if root:
                self.root_node_inbound = root
                self.nodes_prod_inbound = {n.name: n for n in self._walk_nodes(root)}


    # --- envをGUIへバインドする箇所（_bind_env_to_gui）で dict を構築 ---
    def _bind_env_to_gui_OLD(self, env):
        # ...既存の処理...
        ...
        if not self.product_name_list:
            return
        if not getattr(self, "product_selected", None):
            self.product_selected = self.product_name_list[0]

        # ルート取得
        root_ot = getattr(env, "prod_tree_dict_OT", {}).get(self.product_selected)
        root_in = getattr(env, "prod_tree_dict_IN", {}).get(self.product_selected, root_ot)

        self.root_node_outbound = root_ot
        self.root_node_inbound = root_in

        # ★ここで dict を必ず構築（名前を nodes_prod_* に統一）
        self.nodes_prod_outbound = {n.name: n for n in self._walk_nodes(root_ot)} if root_ot else {}
        self.nodes_prod_inbound  = {n.name: n for n in self._walk_nodes(root_in)} if root_in else {}

        # PulldownなどUIへ値をセット（略）
        # ...
        # 初期描画
        #self.show_network_by_product(self.product_selected)       # ←もしあれば
        self.show_psi_by_product("outbound", "demand", self.product_selected)
    ...
    def on_product_changed(self, event):
        ...
        # ★ 追加：root/out/in をセット
        self._apply_selected_product(prod)
    ...
    def on_product_changed(self, event):
        """プルダウン変更時のハンドラ：辞書再構築 → ネットワーク＆PSI再描画。"""
        selected = None
        if hasattr(self, "cb_product"):
            try:
                ...
                pass
        if not selected:
            # 直接呼ばれた場合などは既存選択 or 先頭で代替
            selected = getattr(self, "product_selected", None) or (self.product_name_list[0] if self.product_name_list else None)
    ...
    def setup_ui(self):
        ...
        # --- View menu: 切替を明示 ---
        view_menu.add_command(label="Network Graph", 
                            command=self.show_network_graph)
        view_menu.add_command(label="World Map (highlight product)", 
                            command=lambda: self.show_world_map(self.product_selected))



        # ▼ 追加：World Map のフィット切替（必ず self に保持する）
        self.world_map_fit_var = getattr(self, "world_map_fit_var", None) or tk.BooleanVar(value=True)
        self.world_map_fit = bool(self.world_map_fit_var.get())  # 実体（bool）も保持しておく
...
# …が意外とデリケートなポイント
    ...
    def show_world_map_OLD(self, product_name=None, fit="auto", pad_deg=3.0):
        """
        世界地図ビュー（左パネル）。
        - 全物理ノードを薄灰のエッジで描画
        - 選択productのエッジを赤でハイライト
        - fit="auto" ならノード群に自動フィット（pad_deg度だけ余白）
        fit="global" なら全世界表示
        """
        import matplotlib.pyplot as plt
        ...
        if G is None:
            print("[WARN] show_world_map: self.G is None"); return

        # 製品ハイライト用の (parent, child) 集合
        highlight_edges = set()
        if product_name:
            root = (getattr(self, "prod_tree_dict_OT", {}) or {}).get(product_name)
            if root:
        ...
        if ax is None and hasattr(self, "view_nx_matlib4opt"):
            ...
            ax = getattr(self, "ax_network", None)
        if ax is None:
            print("[WARN] show_world_map: network axes missing."); return

        ax.clear()
        ax.set_title("Global Supply Chain Map", fontsize=12)

        # cartopy があれば地理投影、無ければ普通のAxes
        used_cartopy = False
        try:
        ...
        # 物理ノード辞書（outbound/inbound を合成）
        phys_nodes = {}
        for d in (getattr(self, "nodes_outbound", {}) or {}, getattr(self, "nodes_inbound", {}) or {}):
            phys_nodes.update(d)
        ...
        # 位置と描画
        hub_names = {"sales_office", "procurement_office", "supply_point"}
        pos, xs, ys = {}, [], []
        for name, node in phys_nodes.items():
            x, y = _lon(node), _lat(node)
            pos[name] = (x, y)
            xs.append(x); ys.append(y)
            color = "#1f77b4" if name not in hub_names else "#444444"
            size = 18 if name not in hub_names else 28
            kwargs = dict(marker="o", color=color, markersize=size/3)
            if used_cartopy:
                ax.plot(x, y, transform=ax.projection, **kwargs)
                ax.text(x, y, f" {name}", fontsize=8, transform=ax.projection)
            else:
                ax.plot(x, y, **kwargs)
                ax.text(x, y, f" {name}", fontsize=8)

        # 表示範囲：auto-fit or global
        if xs and ys:
            if fit == "auto":
                xmin, xmax = min(xs)-pad_deg, max(xs)+pad_deg
                ymin, ymax = min(ys)-pad_deg, max(ys)+pad_deg
                if used_cartopy:
        ...
        # エッジ描画
        def _draw_edge(nm_u, nm_v, color, lw, alpha=0.7):
            if nm_u not in pos or nm_v not in pos: return
            x1, y1 = pos[nm_u]; x2, y2 = pos[nm_v]
            if used_cartopy:
                ax.plot([x1, x2], [y1, y2], color=color, linewidth=lw, alpha=alpha, transform=ax.projection)
            else:
                ax.plot([x1, x2], [y1, y2], color=color, linewidth=lw, alpha=alpha)

        # 全エッジ（薄灰）
        try:
            edges_iter = list(G.edges())  # EdgeView→list
        except Exception:
            edges_iter = []
        for u, v in edges_iter:
            _draw_edge(u, v, "#cccccc", 1.2, alpha=0.6)

        # 製品ハイライト（赤）
        for u, v in highlight_edges:
            _draw_edge(u, v, "red", 2.0, alpha=0.9)
    ...
    def show_world_map(self, product_name=None):
        ...
        # 1) 物理ノード集合（全製品の和集合）を SQL 環境から集める
        nodes_all = {}
        ...
        #print("[WM] sample CS_CAL lat/lon:", 
        #    getattr(nodes_all.get("CS_CAL"), "lat", None),
        #    getattr(nodes_all.get("CS_CAL"), "lon", None))


        ## 2) ハイライト（選択製品の PlanNode エッジ）
        #highlight_edges = set()
        ...
        # --- ここまでで used_cartopy, ax, pos が用意済みとする ---
            ...
            if arrow:
                ax.annotate(
                    ...
                    arrowprops=dict(
                        ...
                        shrinkA=0, shrinkB=0, mutation_scale=10, alpha=0.9
                    ),
                    transform=(ccrs.PlateCarree() if used_cartopy and ccrs else ax.transData),
                    zorder=z
                )
            else:
                kw = dict(color=color, lw=lw, alpha=0.7, zorder=z)
                if used_cartopy and ccrs:
                    ax.plot([x1, x2], [y1, y2], transform=ccrs.PlateCarree(), **kw)
                else:
                    ax.plot([x1, x2], [y1, y2], **kw)

        # 3) エッジ描画：全体(薄灰)＋製品ハイライト(赤・矢印)
        # 全エッジ（薄灰）：arrow=False
        for (u, v) in G.edges():
            _seg(u, v, "#cccccc", 1.0, arrow=False, z=3)

        # 製品ハイライト（赤）：arrow=True
        for (u, v) in highlight_edges:
        ...
        ax = getattr(self, "ax_network", None)
        if ax is None and hasattr(self, "view_nx_matlib4opt"):
            self.view_nx_matlib4opt(); ax = getattr(self, "ax_network", None)
        if ax is None: 
            print("[WORLD-MAP] no axes"); return
        ...
        try:
            ...
            ax = fig.add_subplot(111, projection=ccrs.PlateCarree()); self.ax_network = ax
            ax.add_feature(cfeature.LAND.with_scale('50m'), facecolor="#f2f2f2")
            ax.add_feature(cfeature.OCEAN.with_scale('50m'), facecolor="#e6f2ff")
            ax.add_feature(cfeature.BORDERS.with_scale('50m'), linewidth=0.3)
            ax.add_feature(cfeature.COASTLINE.with_scale('50m'), linewidth=0.3)
            ax.set_global(); used_cartopy = True
        except Exception:
        ...
        for name, node in nodes_all.items():
            x, y = _lon(node), _lat(node); pos[name]=(x,y)
            color = "#1f77b4" if name not in hub else "#444444"
            ms = 15 if name not in hub else 30
            if used_cartopy: ax.plot(x,y,"o",color=color,ms=ms/3,transform=ax.projection)
            else:            ax.plot(x,y,"o",color=color,ms=ms/3)
            ax.text(x, y, f" {name}", fontsize=8, va="bottom", ha="left",
                    transform=ax.projection if used_cartopy else ax.transData)

        # 6) 全エッジ（薄灰）: 物理グラフ self.G があれば使う
        G = getattr(self, "G", None)

        #def _seg(u,v,color,lw):
        #    if u not in pos or v not in pos: return
        #    x1,y1=pos[u]; x2,y2=pos[v]
        #    if used_cartopy: ax.plot([x1,x2],[y1,y2],color=color,lw=lw,alpha=0.7,transform=ax.projection)
        #    else:            ax.plot([x1,x2],[y1,y2],color=color,lw=lw,alpha=0.7)

        def _seg(u, v, color, lw, arrow=False):
            if u not in pos or v not in pos: return
            x1, y1 = pos[u]; x2, y2 = pos[v]
            if used_cartopy:
                if arrow:
                    ...
                    ax.plot([x1,x2],[y1,y2], color=color, lw=lw, alpha=0.8, transform=ax.projection)
            else:
                if arrow:
                    ax.annotate("", xy=(x2,y2), xytext=(x1,y1),
                                arrowprops=dict(arrowstyle="->", color=color, lw=lw))
        ...
        # 8) 自動フィット（緯度経度の分布に合わせる）
        if pos:
            xs=[x for x,_ in pos.values()]; ys=[y for _,y in pos.values()]
            xmin,xmax=min(xs),max(xs); ymin,ymax=min(ys),max(ys)
            if used_cartopy:
                ax.set_extent([xmin-10, xmax+10, ymin-5, ymax+5], crs=ax.projection)
            else:
                ax.set_xlim(xmin-10, xmax+10); ax.set_ylim(ymin-5, ymax+5)
    ...
    def show_world_map(self, product_name=None):
        ...
        - 全エッジ：薄灰
        - 選択製品のエッジ：赤（矢印）
        座標は Node.lon / Node.lat を使用。
        """
        # 0) 物理ノード集合を作る（SQL 環境優先）
        env = getattr(self, "psi", None)
        nodes_all = {}
        if env and getattr(env, "prod_tree_dict_OT", None):
            for _prod, _root in env.prod_tree_dict_OT.items():
        ...
        for name, node in nodes_all.items():
            ...
            ms = 15 if name not in hub else 30
            if used_cartopy:
                ax.plot(x, y, "o", color=color, ms=ms/3, transform=ax.projection)
                ax.text(x, y, f" {name}", fontsize=8, va="bottom", transform=ax.projection)
            else:
                ax.plot(x, y, "o", color=color, ms=ms/3)
                ax.text(x, y, f" {name}", fontsize=8, va="bottom")

        # 4) 全エッジ集合（self.G が無ければ製品ツリー総当りで作る）
        all_edges = set()
        G = getattr(self, "G", None)
        ...
        try:
            import cartopy.crs as ccrs  # 既にimport済みなら使われる
        except Exception:
            ccrs = None
            ...
            if used_cartopy and ccrs:
                if arrow:
                    ax.annotate("",
                        xy=(x2, y2), xytext=(x1, y1),
                        arrowprops=dict(arrowstyle="-|>", color=color, lw=lw,
                                        mutation_scale=10, shrinkA=0, shrinkB=0, alpha=0.9),
                ...
                if arrow:
                    ...
                    ax.plot([x1, x2], [y1, y2], color=color, lw=lw, alpha=0.7, zorder=z)
        ...
        if pos:
            ...
            if used_cartopy and ccrs:
                ax.set_extent([xmin-10, xmax+10, ymin-5, ymax+5], crs=ccrs.PlateCarree())
            else:
