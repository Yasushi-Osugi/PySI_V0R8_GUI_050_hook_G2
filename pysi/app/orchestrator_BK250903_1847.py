...
def _report_one_OLD(conn, scenario: str, node_name: str, product_name: str,
...
def _report_one(conn, scenario: str, node_name: str, product_name: str,
    ...
    try:
        ...
        if "outpath" in params:
            ...
            # 例: plot_weekly(df, title, out_file)
            plot_weekly(df, title, str(out))
        elif len(params) == 2:
            # 例: plot_weekly(df, out_file)
            plot_weekly(df, str(out))
        else:
            # 返り値が fig の場合に備えたフォールバック
            res = plot_weekly(df, title=title)
            try:
                # matplotlib Figure なら savefig する
                res.savefig(str(out))
            except Exception:
                # それも無理なら最後の手段で pyplot 保存
    ...
    with _open(args.db) as conn:
        ...
        if args.mode == "leaf":
            ...
            for node_name, product_name in sorted(pairs):
                # ① S生成（demandレイヤのスロットを起こす）
                try:
                    compute_leaf_S_for_pair(conn, scenario_id=sid,
                                            node_name=node_name, product_name=product_name)
                except Exception as e:
                    # 必要ならログ出力
                    pass

                # ② 書戻し（demand/supply両レイヤ）
                res = {}
                if args.write_all_nodes:
                    try:
                        res = write_both_layers_for_pair(conn, scenario_id=sid,
                                                        node_name=node_name, product_name=product_name)
                    except Exception:
                        res = {}
                ...
                if isinstance(res, dict):
                    item.update(res)   # ここに d_rows / s_rows が入る実装なら取り込む
                written.append(item)
        ...
        if args.mode == "tree":
            ...
            # フォールバック：何も無ければ (root.name, 既定product)
            if not pairs:
            ...
            # 計算＆書戻し：現行の keyword-only 版に合わせた呼び出し
            written = []
            sid = get_scenario_id(conn, args.scenario)

            # S生成（必要なら。write_both_layers が内包していれば省略可）
            for node_name, product_name in pairs:
                try:
                    # 先にSを起こしたい場合
                    #@STOP
                    #from pysi.io.tree_writeback import compute_leaf_S_for_pair, write_both_layers_for_pair

                    compute_leaf_S_for_pair(conn, sid, node_name, product_name)
                    res = write_both_layers_for_pair(conn, sid, node_name, product_name)
                except Exception as e:
                    res = {}
                item = {"node": node_name, "product": product_name}
                if isinstance(res, dict): item.update(res)
                written.append(item)
