...
def set_df_Slots2psi4demand_OLD(node, df_weekly):
...
#
    ...
    if node.children == []:  # 子nodeがないleaf nodeの場合
        ...
        # ISO週→内部インデックスの写像を構築
        plan_year_st = getattr(node, "plan_year_st", None)
        plan_range   = getattr(node, "plan_range", None)

        # フォールバック（無ければ df_weekly 最小年と 53換算）
        if plan_year_st is None:
            plan_year_st = int(df_weekly["iso_year"].min())
        if plan_range is None:
            # 既に PSI スペースが確保されている前提なので、長さから概算
            weeks_count = len(getattr(node, "psi4demand", [])) or 53
            plan_range = max(1, (weeks_count + 52) // 53)

        week_index_map, weeks_total = _build_iso_week_index_map(int(plan_year_st), int(plan_range))
        weeks_count = len(node.psi4demand)  # あなたの実装は固定長の list なのでこれを使用

        # 週インデックスに合わせて pSi（lot配列）を作成
        pSi = _make_lot_id_list_slots_iso(df_weekly, node.name, week_index_map, weeks_count)
...
# 後ろに同じ関数あり
def propagate_postorder_with_calcP2S_OLD(root, *, layer="demand",
                                     parent_before_child=True, dedup=True):
    """
    完全 post-order 1パス：
      1) 葉で S->P
    ...
    for n in nodes:
        ...
        # 親：子P->親S
        if hasattr(n, "calcP2S"):
            n.calcP2S(layer=layer,
                      parent_before_child=parent_before_child,
                      dedup=dedup)
        else:
            # 既存のユーティリティがあればフォールバック
            try:
                from pysi.plan.operations import get_set_childrenP2S2psi
                get_set_childrenP2S2psi(n)   # 1-hop 集約想定
            except Exception:
                pass
