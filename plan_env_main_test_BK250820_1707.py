...
# 追加
from pysi.plan.operations import propagate_postorder_with_calcP2S, set_df_Slots2psi4demand
...
def main():
    ...
    def assert_unique_within_node_bucket(root, bucket_label="S", layer="demand", limit=20):
        """
        同一 node × 同一 bucket（S/P）内で、lot_id が週を跨いで重複しないことを検査。
        CO/I は対象外（滞留OK）。
        """
        bidx = BUCKET[bucket_label]
        total = 0
        violations = []

        for nd in _walk(root):
            psi = nd.psi4supply if layer == "supply" else nd.psi4demand
            if not isinstance(psi, list):
                continue
            seen = set()
            for w in range(len(psi)):
                for lot in psi[w][bidx]:
                    total += 1
                    if lot in seen:
                        violations.append((nd.name, bucket_label, w, lot))
                    else:
                        seen.add(lot)

        if violations:
            print(f"[WARN] within-node '{bucket_label}' duplicates: {len(violations)} (show {min(limit,len(violations))})")
            for rec in violations[:limit]:
                print("  - node,bucket,week,lot:", rec)
        else:
            print(f"[OK] within-node '{bucket_label}' uniqueness verified. total={total}")
        return total, len(violations), violations[:limit]

    def check_leafS_vs_rootP(root):
        """
        葉ノード群の S 総数 と root の P 総数の整合チェック。
        休暇/レンジ外押し出しが無ければ一致が期待値。
        """
        leaves = _leaves(root)
        leaf_S = 0
        for lf in leaves:
            ...
            if not isinstance(psi, list):
                continue
            leaf_S += sum(len(psi[w][BUCKET["S"]]) for w in range(len(psi)))

        psi_root = root.psi4demand
        root_P = sum(len(psi_root[w][BUCKET["P"]]) for w in range(len(psi_root)))

        print(f"sum(leaf S) = {leaf_S}")
        print(f"sum(root P) = {root_P}")
        if leaf_S != root_P:
            print(f"[INFO] leaf S != root P (SS/LT/休暇でのレンジ外押し出し等で差があり得ます). Δ={root_P-leaf_S}")
        else:
            print("[OK] leaf S == root P")
    ...
    for prod, root in psi.prod_tree_dict_OT.items():
        ...
        # ---- ここから検証出力 ----
        leaves = _leaves(root)
        S_parent = _psi_counts(root, bucket_idx=0, layer="demand")
        P_leaves_total = sum(_psi_counts(lf, bucket_idx=3, layer="demand") for lf in leaves)
        ...
        # ① 葉S総数 と rootP総数の整合
        check_leafS_vs_rootP(root)

        # ② 同一 node×bucket 内ユニーク（S と P をチェック）
        assert_unique_within_node_bucket(root, bucket_label="S", layer="demand")
        assert_unique_within_node_bucket(root, bucket_label="P", layer="demand")
